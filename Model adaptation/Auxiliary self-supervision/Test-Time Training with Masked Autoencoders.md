...
本文对于 TTT 为何有效给出了具有说明性的见解：在分布变化下，TTT 找到了比应用一个固定的模型更好的偏差-方差权衡（bias-variance trade-off）。固定模型存在偏差因为其完全基于无法代表新的测试分布的偏差训练数据。另一种极端是完全遗忘训练数据，并在每个测试输入上从头开始训练一个新的模型，这也是不切实际的，因为单个测试输入是高方差的，尽管定义是无偏的
